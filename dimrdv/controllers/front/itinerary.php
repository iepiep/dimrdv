...
class DimrdvItineraryModuleFrontController extends ModuleFrontController {
    ...
    private function calculateOptimizedRoute($selectedIds) {
        ...
        foreach ($data['rows'] as $i => $row) {
            foreach ($row['elements'] as $j => $element) {
                $distanceMatrix[$i][$j] = ($element['status'] === 'OK') ? $element['distance']['value'] : PHP_INT_MAX;
                $durationMatrix[$i][$j] = ($element['status'] === 'OK') ? $element['duration']['value'] : PHP_INT_MAX;
            }
        }
